...
def handle_file_upload(request):
    ...
    for file in files:
        ...
        filename = f.split(".")[0]
        try:
            folder_name_accrd_file = list(
                Folder.objects.filter(file__file_name__iexact=filename).values()
            )[0]
            if folder_name_accrd_file.get("folder_name") == folder_name:
                return Response(
                    {"st": "same file exist in same folder"},
    ...
    if folder_name is "" and len(document_topics) == 0 and len(file_names) == 0:
        ...
        try:
            ...
            if len(final_response[0]) == 0:
                return Response(
                    {"st": "no such file found"}, status=status.HTTP_400_BAD_REQUEST
                )
        ...
        try:
            for tp in document_topics:
                t = FileUpload.objects.filter(file_topic__topic_name__iexact=tp).filter(
                    f2f__folder_name__iexact=folder_name
                )
                final_response.append(FileSerializers(t, many=True).data)
            if len(final_response[0]) == 0:
                return Response(
    ...
    for f_name in file_names:
        try:
            if folder_name is "":
                ...
                file_obj1 = FileUpload.objects.filter(file_name__iexact=f_name).filter(
                    f2f__folder_name__iexact=folder_name
                )

            file_topics_all_dict = list(
                Topics.objects.filter(topics__file_name__iexact=f_name).values("id")
            )
            ...
            return Response(
                {"st": "server error"}, status=status.HTTP_503_SERVICE_UNAVAILABLE
            )
    return Response(final_response, status=status.HTTP_200_OK)
    ...
    if folder_name != "" and len(file_names) == 0:
        try:
            ...
            if folder_qury_set.exists():
                ...
                return Response(
                    {"msg": "folder removed successfully"}, status=status.HTTP_200_OK
                )
            else:
                return Response(
                    {"msg": "such folder doest not exist."},
        ...
        if len(success_file_names) == 0:
            ...
            return Response(
                {
                    "deleted_files": success_file_names,
                    "msg": "files delete successfully.x",
                },
        ...
        try:
            ...
            if folder_qury_set.exists():
                ...
                for file in file_names:
                    ...
                    folder_file_rel_query = FileUpload.objects.filter(
                        f2f__folder_name__iexact=folder_name
                    ).filter(file_name__iexact=file)
                    try:
                        if list(folder_file_rel_query.values())[0] is not None:
                            ...
                            folder_find_status = (
                                Folder.objects.filter(file__file_name__iexact=file)
                                .filter(~Q(folder_name__iexact=folder_name))
                                .exists()
                            )
                ...
                if len(success_file_names) > 0:
                    return Response(
                        {
                            "deleted_files": success_file_names,
                            "msg": "files are deleted successfully.",
                        },
                        status=status.HTTP_200_OK,
                    )
                else:
                    return Response(
                        {"msg": "such folder/files doest not exist."},
                        status=status.HTTP_204_NO_CONTENT,
                    )
            else:
                return Response(
